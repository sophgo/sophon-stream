...
struct QualityConfig {
    ...
    QualityConfig(const float ta, const float trl, const float tru, const float w,
        ...
        margin(mg), maxScore(maxSc), lateralSide(ls) {
    ...
    float unit;/**< 单位(需要换算到0.0f~100.0f) 通过(upArea-lowArea)/100计算得出 */
  ...
  private:
    ...
    */
    float boxFilter(const cv::Rect2f &rect, bool &outRange) {
        ...
        if (area < mQualityConfig.thresArea) {
            outRange = false;
            return 0.0f;
        } else if (ratio < mQualityConfig.thresRatioLowBound || ratio > mQualityConfig.thresRatioUpBound) {
            outRange = false;
            return 0.0f;
        } else {
            outRange = true;
    ...
    */
    float centerScore(const cv::Rect2f &rect, bool &outRange) {
        ...
        float score = (1 - (pow((point.x - mQualityConfig.center.x), 2) + pow((point.y - mQualityConfig.center.y), 2))
                       / (pow(mQualityConfig.center.x, 2) + pow(mQualityConfig.center.y, 2))) * 100;
        if (point.y <= (mQualityConfig.height / 2)) { //上部
            score *= 0.8f;
        }//else为下部

        (rect.tl().x > mQualityConfig.margin && rect.tl().y > mQualityConfig.margin &&
         rect.br().x < (mQualityConfig.width - mQualityConfig.margin) &&
         rect.br().y < (mQualityConfig.height - mQualityConfig.margin)) ? (outRange = true) :
        (outRange = false);
    ...
    *
    */
    void ldValid(const std::vector<cv::Point2f> &landmarks, const cv::Rect2f &rect, bool &outRange) {
    ...
    */
    float verticalDis(const std::vector<cv::Point2f> &landmarks, bool &outRange) {
        ...
        for(unsigned int i=0; i<4; i++) {
            ...
            float side = sqrt(pow((vecs[i].x-vecs[b].x),2) + pow((vecs[i].y-vecs[b].y),2));
        ...
        // 左右
        float sx = (hghts[0] < hghts[2])? (hghts[0]/hghts[2]) : (hghts[2]/hghts[0]);
        // 上下
        float sy = (hghts[1] < hghts[3])? (hghts[1]/hghts[3]) : (hghts[3]/hghts[1]);
        // 有效性判断 越小 越宽松
    ...
    *
    */
    float includeAngle(const std::vector<cv::Point2f> &landmarks, bool &outRange) {
        ...
        for(unsigned int i=0; i<3; i=i+2) {
            ...
            float mul = sqrt(pow(dist[i].x,2) + pow(dist[i].y,2)) * sqrt(pow(dist[i+1].x,2) + pow(dist[i+1].y,2));
            double cos = dot / mul;
        ...
        for(unsigned int i=0; i<2; i++) {
            ...
            float mul = sqrt(pow(dist[i].x,2) + pow(dist[i].y,2)) * sqrt(pow(dist[i+2].x,2) + pow(dist[i+2].y,2));
            double cos = dot / mul;
            cos = (cos>0.999) ? 0.999:(cos<-0.999) ? -0.999:cos;
            float temp = (acos(cos) *180/M_PI)/360.0;
            beta.push_back(temp);
        ...
        /**/
        float distLR = fabs(beta[0]-beta[1]); //理论上相等，不相等，则说明左右有偏转
        float distUp = alpha[0] - (beta[0]>beta[1]?beta[0]:beta[1]); //理论上不超过0,若超过说明向上仰头
        ...
        if ((distLR>lRThreshold) || (distUp>upThreshold) || (distUD>doThreshoold)) {
            outRange = false;
            return 0;
        }
        //计算得分

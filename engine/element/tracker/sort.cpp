...
* @param[in]   fc    回调函数
*/
void TrackerChannels::process(std::shared_ptr<common::ObjectMetadata> objectMetadata, const PUT_TASK_FUNC &fc) {

    auto &&iter = mSorts.find(objectMetadata->mFrame->mChannelId);
    if (iter == mSorts.end()) {
        //没找到跟踪器，则创建一个跟踪器，进行跟踪,并添加到map中
        std::shared_ptr<TrackerChannel> sort = std::make_shared<TrackerChannel>(*pQualityControl, mTopN, mIou,
                                               mMaxAge,
                                               mMinHins, mUpdateTimes);

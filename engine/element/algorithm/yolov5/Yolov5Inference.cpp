...
common::ErrorCode Yolov5Inference::init(Yolov5SophgoContext& context) {
  ...
  if (pSophgoContext->use_tpu_kernel) {
    ...
    pSophgoContext->func_id =
        tpu_kernel_get_function(pSophgoContext->m_bmContext->handle(),
                                tpu_module, "tpu_kernel_api_yolov5_detect_out");
    ...
    for (int i = 0; i < input_num; i++)
      in_dev_mem[i] =
          *pSophgoContext->m_bmNetwork->outputTensor(i)->get_device_mem();

...
#define RETINA_FACE_ID 7001
#define YOLO_HEAD 7002
...
TEST(TestMultiAlgorithmGraph, MultiAlgorithmGraph) {
    ...
    workersConfigure.push_back(makeWorkerConfig(YOLO_HEAD, "action_worker", "nvidia", 0, 1, 200, false, 8, {yolov3HeadJson}));
...
//    {"fc1"}, {1}, {{202, 1, 1}}, {0.8},2);
    ...
    graphConfigure["connections"].push_back(makeConnectConfig(RETINA_FACE_ID,0,YOLO_HEAD,0));
    graphConfigure["connections"].push_back(makeConnectConfig(YOLO_HEAD,0,ENCODE_ID,0));
    ...
    engine.setDataHandler(1,REPORT_ID,0,[&](std::shared_ptr<void> data) {
        ...
        if(objectMetadata->mPacket!=nullptr&&objectMetadata->mPacket->mData!=nullptr){
            ...
            for(int i=0;i<objectMetadata->mSubObjectMetadatas.size();i++){
                ...
                if(detectData->mLabelName=="faceDetect"){

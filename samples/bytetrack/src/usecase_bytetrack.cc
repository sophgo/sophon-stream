...
void draw_tracker_bmcv(bm_handle_t& handle, int classId,
                       std::vector<std::string>& class_names, int track_id,
                       float conf, int left, int top, int width, int height,
                       bm_image& frame,
...
TEST(TestMultiAlgorithmGraph, MultiAlgorithmGraph) {
  ...
  std::string bytetrack_config_file = "../config/bytetrack.json";
  ...
  if (bytetrack_json.class_names.size() == 0) {
    IVS_WARN("can not read coco.names");
  }
  int num_channels =
  ...
  for (auto& graph_it : engine_json) {
    if (graph_url_idx == bytetrack_json.num_graphs) break;
    ...
    parse_element_json(elements_it, elementsConfigure, src_id_port,
                       sink_id_port);
    printf("sink_id_port:  %d, %d", sink_id_port.first, sink_id_port.second);
    ...
    for (int channel_id = 0; channel_id < bytetrack_json.num_channels_per_graph;
      ...
      nlohmann::json decodeConfigure =
          bytetrack_json.decodeConfigures[channel_id];

...
nlohmann::json makeDecoderWorkerConfig(int workerId,std::string workerName,
    ...
    nlohmann::json moduleConfigure;
    moduleConfigure["skip_count"] = skipCount; 
...
//    auto& engine = lynxi::ivs::framework::SingletonEngine::getInstance();
    ...
    workersConfigure.push_back(makeDecoderWorkerConfig(DECODE_ID,"decoder_worker","nvidia",0,1,0,false,1,1, "../lib/libmultiMediaApi.so" ));
    ...
    workersConfigure.push_back(makeWorkerConfig(RETINA_FACE_ID, "action_worker", "nvidia", 0, 1, 200, false, 12, {retinafaceJson}));
    ...
    workersConfigure.push_back(makeWorkerConfig(ATTRIBUTE_FACE_ID,"action_worker","nvidia",0,1,200,true,35, {attributeJson}));
...
//    graphConfigure["connections"].push_back(makeConnectConfig(0,0,REPORT_ID,0));
    ...
    auto operateChannelErrOutput = graphOperateRequest.mutable_graph()->add_output_worker_ports();
    operateChannelErrOutput->set_worker_id(DECODE_ID);
    operateChannelErrOutput->set_worker_port(2);
...
//    engine.addGraph(graphConfigure.dump());
    ...
    ::grpc::ClientContext recieveErrChannelContext;
    lynxi::ivs::grpc::ReceiveRequest errRecieveRequest;
    errRecieveRequest.set_graph_id(1);
    errRecieveRequest.mutable_report_worker_port()->set_worker_id(DECODE_ID);
    errRecieveRequest.mutable_report_worker_port()->set_worker_port(2);
    auto clientErrReader = stub->ReceiveErrChannel(&recieveErrChannelContext, errRecieveRequest);

    std::shared_ptr<std::thread> recieveErrThread
        = std::make_shared<std::thread>([&clientErrReader, &cv](){
                lynxi::ivs::grpc::ChannelOperateResponse channelErrResponse;
                while(clientErrReader->Read(&channelErrResponse)){
                    IVS_ERROR("recieve error by channel:{0}, error code is {1}", channelErrResponse.channel_id(), 
                            channelErrResponse.error_code());
                }
                });

...
common::ErrorCode Dwa::dwa_gdc_work(
  ...
  if (dwaObj != nullptr) {
    ...
    if (is_resize == true) {
      ...
      // bm_image_write_to_bmp(*resized_img, "resized_img.bmp");
      bmcv_dwa_gdc(dwaObj->mFrame->mHandle, *resized_img, *dwa_image, ldc_attr);
      // bm_image_write_to_bmp(*dwa_image, "dwa_image.bmp");
  ...
  // bm_image_write_to_bmp(*dwaObj->mFrame->mSpDataDwa, "dwa_image.bmp");
  // bm_image_write_to_bmp(*dwaObj->mFrame->mSpData, "origin_dwa_image.bmp");

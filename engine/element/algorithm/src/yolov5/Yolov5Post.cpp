...
namespace sophon_stream
{
  namespace algorithm
  {
    namespace yolov5
    {
      ...
      void Yolov5Post::setTpuKernelMem(Yolov5SophgoContext& context, common::ObjectMetadatas &objectMetadatas)
      {
        Yolov5SophgoContext* pSophgoContext = &context;
      ...
      // tpu-kernel
      void Yolov5Post::postProcess(Yolov5SophgoContext& context, common::ObjectMetadatas &objectMetadatas)
      {
        ...
        if(pSophgoContext->use_tpu_kernel)
        {
          ...
          for (int i = 0; i < pSophgoContext->max_batch; i++)
          {
            ...
            float ratio = get_aspect_scaled_ratio(image.width, image.height, pSophgoContext->m_net_w, pSophgoContext->m_net_h, &isAlignWidth);
            if (isAlignWidth)
            {
        ...
        else
        {
          ...
          for(int batch_idx = 0; batch_idx < pSophgoContext->max_batch; ++ batch_idx)
          {
              ...
              #ifdef USE_ASPECT_RATIO
                  bool isAlignWidth = false;
                  float ratio = get_aspect_scaled_ratio(frame_width, frame_height,

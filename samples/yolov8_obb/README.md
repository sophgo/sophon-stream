...
```bash
./datasets
└── DOTAv1 #DOTAv1验证集，去掉了长或宽大于8192的图片。
```
...
```json
{
  "channels": [
    {
      "channel_id": 2,
      "url": "../yolov8_obb/data/datasets/DOTAv1/images/val",
      "source_type": "IMG_DIR",
      "sample_interval": 1,
      "loop_num": 1,
      "fps": -1
...
需要注意，部署环境下的NPU等设备内存大小会显著影响例程运行的路数。如果默认的输入路数运行中出现了申请内存失败等错误，可以考虑把输入路数减少，如果无法再减少，则可以调低`sophon-stream/framework/src/datapipe.cc`中的宏定义`DEFAULT_DATA_PIPE_CAPACITY`再进行测试。
...
目前，yolov8_obb例程支持在BM1684X的PCIe、SoC模式，BM1688的SoC模式下进行推理。

在不同的设备上可能需要修改json配置，例如模型路径、输入路数等。json的配置方法参考6.1节，程序运行方法参考上文6.2节。

由于PCIe设备cpu能力差距较大，性能数据没有参考意义，这里只给出SoC模式的测试结果。

测试数据集`DOTAv1/images/val`，编译选项为Release模式，测试`yolov8s-obb_fp16_1b.bmodel`模型性能，结果如下:

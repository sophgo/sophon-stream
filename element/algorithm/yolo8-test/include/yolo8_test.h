...
#ifndef SOPHON_STREAM_ELEMENT_YOLO8_TEST_H_
#define SOPHON_STREAM_ELEMENT_YOLO8_TEST_H_
 ...
 * 算法模块
 */
class Yolo8Test : public ::sophon_stream::framework::Element {
 public:
  Yolo8Test();
  ~Yolo8Test() override;
 ...
 private:
  std::shared_ptr<Yolo8TestContext> mContext;          // context对象
  std::shared_ptr<Yolo8TestPreProcess> mPreProcess;    // 预处理对象
  std::shared_ptr<Yolo8TestInference> mInference;      // 推理对象
  std::shared_ptr<Yolo8TestPostProcess> mPostProcess;  // 后处理对象
...
#endif  // SOPHON_STREAM_ELEMENT_YOLO8_TEST_H_
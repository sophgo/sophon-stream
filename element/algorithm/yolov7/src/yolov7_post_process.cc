...
void Yolov7PostProcess::init(std::shared_ptr<Yolov7Context> context) {
...
int Yolov7PostProcess::argmax(float* data, int num) {
  float max_value = 0.0;
  int max_index = 0;
  for (int i = 0; i < num; ++i) {
    float value = data[i];
...
float Yolov7PostProcess::sigmoid(float x) { return 1.0 / (1 + expf(-x)); }

float Yolov7PostProcess::get_aspect_scaled_ratio(int src_w, int src_h,
                                                 int dst_w, int dst_h,
                                                 bool* pIsAligWidth) {
  float ratio;
...
void Yolov7PostProcess::NMS(YoloV7BoxVec& dets, float nmsConfidence) {
  int length = dets.size();
  int index = length - 1;

  std::sort(
      dets.begin(), dets.end(),
      [](const YoloV7Box& a, const YoloV7Box& b) { return a.score < b.score; });
...
void Yolov7PostProcess::setTpuKernelMem(
...
void Yolov7PostProcess::postProcess(std::shared_ptr<Yolov7Context> context,
...
void Yolov7PostProcess::postProcessTPUKERNEL(
    std::shared_ptr<Yolov7Context> context,
...
void Yolov7PostProcess::postProcessCPU(
    std::shared_ptr<Yolov7Context> context,
    common::ObjectMetadatas& objectMetadatas) {
  if (objectMetadatas.size() == 0) return;

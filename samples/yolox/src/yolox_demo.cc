    ...
    if (yolox_json.download_image) {
      ...
      for (auto detObj : objectMetadata->mDetectedObjectMetadatas) {
        draw_bmcv(objectMetadata->mFrame->mHandle, detObj->mClassify,
                  yolox_json.class_names, detObj->mScores[0], detObj->mBox.mX,

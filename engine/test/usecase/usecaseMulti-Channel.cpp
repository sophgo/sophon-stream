...
//                        }
                ...
                std::string strchannelid = std::to_string(report.object_metadata().packet().channel_id());
                cv::imshow(strchannelid, cpuMat);
                cv::waitKey(1);
            }
    ...
    for(int i=0;i<count;i++){
        ...
        decodeConfigure["url"] =    "rtmp://192.168.3.20:1935/live/instance" + std::to_string(i);//"rtmp://192.168.3.209:1935/live/instance1";
    ...
    //    engine.sendData(1,DECODE_ID,0,data,std::chrono::seconds(4));
        ...
        channelOperateRequest.mutable_channel()->set_channel_id(i);

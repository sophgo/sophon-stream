...
def parse_dev(filepath):
    with open(filepath, 'r') as f:
        ...
        vpp_used_mean = 0 if len(vpp_used_list) == 0 else np.mean(vpp_used_list)
        vpp_rate_mean = 0 if len(vpp_rate_list) == 0 else np.mean(vpp_rate_list)
        vpp_peak = 0 if len(vpp_peak_list) == 0 else np.max(vpp_peak_list)
        npu_used_mean = 0 if len(npu_used_list) == 0 else np.mean(npu_used_list)
        npu_rate_mean = 0 if len(npu_rate_list) == 0 else np.mean(npu_rate_list)
        npu_peak = 0 if len(npu_peak_list) == 0 else np.max(npu_peak_list)
        vpu_used_mean = 0 if len(vpu_used_list) == 0 else np.mean(vpu_used_list)
        vpu_rate_mean = 0 if len(vpu_rate_list) == 0 else np.mean(vpu_rate_list)
        vpu_peak = 0 if len(vpu_peak_list) == 0 else  np.max(vpu_peak_list)

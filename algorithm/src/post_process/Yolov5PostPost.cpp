...
namespace sophon_stream
{
  namespace algorithm
  {
    namespace post_process
    {
      ...
      void Yolov5PostPost::initTpuKernel(algorithm::Context &context, common::ObjectMetadatas &objectMetadatas)
      {
        ...
        std::string tpu_kernel_module_path = "../tpu_kernel_module/libbm1684x_kernel_module.so";
        tpu_module = tpu_kernel_load_module_file(*objectMetadatas[0]->mAlgorithmHandle, tpu_kernel_module_path.c_str());
        pSophgoContext->func_id = tpu_kernel_get_function(*objectMetadatas[0]->mAlgorithmHandle, tpu_module, "tpu_kernel_api_yolov5_detect_out");
      ...
      int Yolov5PostPost::argmax(float *data, int num)
      {
        float max_value = 0.0;
        int max_index = 0;
        for (int i = 0; i < num; ++i)
      ...
      float Yolov5PostPost::sigmoid(float x)
      {
        return 1.0 / (1 + expf(-x));
      }

      void Yolov5PostPost::NMS(Yolov5PostBoxVec &dets, float nmsConfidence)
      {
        int length = dets.size();
        int index = length - 1;

        std::sort(dets.begin(), dets.end(), [](const Yolov5PostBox &a, const Yolov5PostBox &b)
      ...
      void Yolov5PostPost::postProcess(algorithm::Context &context, common::ObjectMetadatas &objectMetadatas)
      {
        context::SophgoContext *pSophgoContext = dynamic_cast<context::SophgoContext *>(&context);
        ...
        if (pSophgoContext->use_tpu_kernel && !pSophgoContext->mEndOfStream)
        {
          ...
          for (int i = 0; i < pSophgoContext->max_batch; i++)
          {
            ...
            bm_memcpy_d2s_partial_offset(*objectMetadatas[0]->mAlgorithmHandle, (void *)pSophgoContext->output_tensor[i], pSophgoContext->out_dev_mem[i], pSophgoContext->detect_num[i] * 7 * sizeof(float), 0); // 25200*7

            for (int bid = 0; bid < pSophgoContext->detect_num[i]; bid++)
            {
              Yolov5PostBox temp_bbox;

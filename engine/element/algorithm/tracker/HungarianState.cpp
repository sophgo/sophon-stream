...
void* step1(HungarianState& state) {
  /*
   * 第一次执行step1的时候(当然,不同于step3到step6,
   * step1在每次更新的时候只会执行一次),
   * 因为默认设置的col_uncovered和row_uncovered都为true, 所以step1的功能是:
   *
   * ① 将state.C的每一行都减去这一行的最小值, 以确保新的state.C的每一行
   *   都有1个或者多个为0的元素.
   * ② 并将state.C中等于0的元素的索引位置于state.marked标记为1.
   * (初始化的时候state.marked的所有元素都为0).
   *
   * */
  // 1) 减去每行的最小值.
...
void* step3(HungarianState& state) {
  /*Cover each column containing a starred zero. If n columns are covered,
    ...
    trackers可以1-1匹配上. 这种情况下, 结束优化, 否则的话,
   进入step4继续执行linear assignment算法.

    所以step3的功能是:
   *
   * ① 创建一个名为marked的矩阵(bool型, shape与state.marked一样.
   还是由二维vector组成.)
   *
   * ② 并根据marked的情况, 对marked的某列中有True的情况,
   将未覆盖的列(col_uncovered)的对应列设置为False(双重否定表肯定).
   * */
...
void* step4(HungarianState& state) {
  ...
  //    对covered_C, 保留col_uncovered中为True的列,
  //    把col_uncovered为False的列置为0.
  ...
  while (1) {
    ...
    if (covered_C[row_coord][col_coord] == 0) {
      ...
      if (state.marked[row_coord][star_col] != 1) {
        ...
        //                covered_C[:, col] = C[:, col] * (
        //                        state.row_uncovered.astype(dtype=np.int,
        //                        copy=False))
        // covered_C中第col列的值都*state.row_uncovered的值,
        // 若对应的位置为False, 则covered_C中 第col列对应的某行的值为0;
        // 若对应位置为True, 不变.
        for (int i = 0; i < rows; i++) {
          if (state.row_uncovered[i] != true) {
...
void* step5(HungarianState& state) {
  ...
  */
  ...
  while (1) {
    ...
    //        row = np.argmax(state.marked[:, path[count, 1]] == 1)
    //    state.marked[:, x] == 1表示判断marked矩阵的第x列的所有值跟1的差别.
    //    返回bool的数组.
    int row = 0;
    ...
    for (int i = 0; i < row_length; i++) {
      ...
      (state.path[count][1] == -1) ? (colIndex = col_length - 1)
                                   : (colIndex = state.path[count][1]);
    ...
    tmp_col_count.assign(state.marked[state.path[count][0]].begin(),
                         state.marked[state.path[count][0]].end());
  ...
  for (int i = 0; i < count + 1; i++) {
    unsigned int colIndex = 0;
    (state.path[i][1] == -1) ? (colIndex = col_length - 1)
                             : (colIndex = state.path[i][1]);

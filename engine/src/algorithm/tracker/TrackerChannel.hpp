...
class TransTrack2Output {
    ...
    static void transTrack2Output(std::vector<std::shared_ptr<common::ObjectMetadata>> &objs, const unsigned long long trackId,
        ...
        for (auto &obj : objs) {
            if (n < topN) {
                ...
                //get current date
                std::string strDay;
                Clocker::getCurrentUs(obj->mTrackedObjectMetadata->mCaptureTime, strDay);
                //obj->mTrackedObjectMetadata->mImagePath =  strDay + "/" + Utils::genUUID() + ".jpg";
                if (n == (limit - 1)) {
                    obj->mTrackedObjectMetadata->mTrackFlag = common::TrLast;
                }
                //IVS_CRITICAL("track image path: {0}", obj->mTrackedObjectMetadata->mImagePath);
                fc(obj);
                n++;
            } else break;
        }
  ...
  private:
    ...
    * @return:从大到小
    */
    static bool sortByScore(const std::shared_ptr<common::ObjectMetadata> &v1, const std::shared_ptr<common::ObjectMetadata> &v2) {
        return v1->mTrackedObjectMetadata->mQualityScore > v2->mTrackedObjectMetadata->mQualityScore;
    }
};
...
class Tracker {
  public:
    ...
    * @return:
    */
    Tracker(cv::Rect rect, unsigned long long &trackId, const int topN): mTopN(topN) { /*:mTrackerId(&trackId)*/
        this->rect = rect;
    ...
    * @return: 跟踪id
    */
    inline unsigned long long getTrackId() {
        return mTrackerId;
    }
    ...
    * @return: mHitStreak
    */
    inline unsigned long long getHitStreak() {
        return mHitStreak;
    }
    ...
    * @return: mObjs
    */
    inline std::vector<std::shared_ptr<common::ObjectMetadata>> &getTrackObject() {
        return mObjs;
    }

    inline void clearObjs() {
        mObjs.clear();
    }
    ...
    */
    ...
    void timebaseFilter(const long timebase, const bool first,
        ...
        if ((mTimer > timebase) && (!first)) {
            ...
            TransTrack2Output::transTrack2Output(mObjs, mTrackerId, fc, mTopN, maxScore);
    ...
    */
    void pushbackObjs(std::shared_ptr<common::ObjectMetadata> &metData) {
        if(mObjs.size()==0) {
            ...
            if(metData->mTrackedObjectMetadata->mQualityScore > mObjs[0]->mTrackedObjectMetadata->mQualityScore) {
                mObjs.clear();
                mObjs.push_back(metData);
            }
        }
...
class TrackerChannel {
  ...
  public:
    ...
    using PUT_TASK_FUNC = std::function<void (std::shared_ptr<common::ObjectMetadata>&)>; //总的metadata
    ...
    */
    ...
    TrackerChannel(const QualityControl &qc, const int topN, const float iouThreshold, const int maxAge = 1,
                   const unsigned long long minHits = 3,
                   const int updateTimes = 0): mQualityControl(qc), mTopN(topN), mIouThreshold(iouThreshold),
        mMaxAge(maxAge), mMinHits(minHits) {}
    ~TrackerChannel() {
        mListTrackers.clear();
    }
    ...
    /**
    * update函数 将每一个目标做了质量评价，放入跟踪其中，新的目标和跟踪器中旧的目标做了
    ...
    */
    ...
    void update(const long timebase, std::shared_ptr<common::ObjectMetadata> &frameMetdata, const PUT_TASK_FUNC &fc) {
        mFrameCount++;
        ...
        for (auto iter = trks.begin(); iter != trks.end();) {
            if (std::isfinite((*iter)[0]) && std::isfinite((*iter)[1]) && std::isfinite((*iter)[2]) &&
                    std::isfinite((*iter)[3]) && std::isfinite((*iter)[4])) {
                iter++;
            } else trks.erase(iter);
        }

        if (frameMetdata->mSubObjectMetadatas.size() != 0) {

//            PayloadVideoFaceRecognize *payload = dynamic_cast<PayloadVideoFaceRecognize *>(frame->mPayload.get());
//            if (payload != nullptr) {
//                if (!payload->mliveStream) {
//                    IVS_DEBUG("SortAlgorithm::update begin hungarian... trakerList size:{0}", mListTrackers.size());
//                }
//            }
            ...
            //iou and hungarian
            associateDetectionsToTrackers(frameMetdata->mSubObjectMetadatas, trks, matches, unmatchedDets, unmatchedTrks, mIouThreshold);

            for (int i = 0; i < mListTrackers.size(); i++) {
                if (std::find(unmatchedTrks.begin(), unmatchedTrks.end(), i) == unmatchedTrks.end()) {
                    int d = -1;
                    ...
                    if (d != -1) {
                        frameMetdata->mSubObjectMetadatas[d]->mTrackedObjectMetadata->mTrackId = mListTrackers[i].getTrackId();
        ...
        for (auto iter = mListTrackers.rbegin(); iter != mListTrackers.rend();) {
            ...
            if (frameMetdata->mFrame->mData == nullptr) {
                if (frameMetdata->mFrame->mEndOfStream) {
                    ...
                    iter = std::vector<Tracker>::reverse_iterator(mListTrackers.erase((++iter).base()));
                } else
                    iter++;
                continue;
            }

            //IVS_TRACE("rectTracker.x:{0}---orig width:{1}", rectTracker.x, frameMetdata->mFrame->mWidth);
            if (iter->getTimeSinceUpdate() >= mMaxAge || rectTracker.x < 0 || rectTracker.y < 0
                    || rectTracker.br().x > frameMetdata->mFrame->mWidth ||
                    rectTracker.br().y > frameMetdata->mFrame->mHeight ||
                    frameMetdata->mFrame->mEndOfStream && iter->getHitStreak() >= mMinHits) {
  ...
  private:
    ...
    */
    float iou(const cv::Rect2f &det, const std::vector<float> &trk) {
        ...
        return (float)areaOverlap / (det.width * det.height + (trk[2] - trk[0]) * (trk[3] - trk[1]) - areaOverlap);
    }

    /**
    * associateDetectionsToTrackers函数 检测到的所有目标框和所有跟踪器中的人脸框先做iou，然后
    * 根据iou进行匈牙利匹配
    ...
    * @return:
    */
    void associateDetectionsToTrackers(const std::vector<std::shared_ptr<common::ObjectMetadata>> &dets,
                                       const std::vector<std::vector<float>> &trks,
                                       std::vector<std::vector<int>> &matches,
                                       std::vector<int> &unmatchedDets,
                                       std::vector<int> &unmatchedTrks,
                                       const float iouThreshold = 0.25) {

...
TEST(TestMultiAlgorithmGraph, MultiAlgorithmGraph) {
  ...
  graphConfigure["connections"].push_back(
      makeConnectConfig(PRE_ID, 0, UNET_ID, 0));
  graphConfigure["connections"].push_back(
      makeConnectConfig(UNET_ID, 0, POST_ID, 0));
  ...
  engine.setDataHandler(1, POST_ID, 0, [&](std::shared_ptr<void> data) {
    ...
    if (objectMetadata == nullptr ||
        objectMetadata->mSegmentedObjectMetadatas.size() == 0)
      return;
    ...
    if (objectMetadata->mSegmentedObjectMetadatas[0]->mFrame != nullptr &&
        objectMetadata->mSegmentedObjectMetadatas[0]->mFrame->mSpData !=
            nullptr) {
      int width = objectMetadata->mSegmentedObjectMetadatas[0]->mFrame->mWidth;
      int height =
          objectMetadata->mSegmentedObjectMetadatas[0]->mFrame->mHeight;
      // 转成bm_image
      bm_image image1 =
          *objectMetadata->mSegmentedObjectMetadatas[0]->mFrame->mSpData;
  ...
  sophon_stream::common::ErrorCode errorCode = engine.sendData(
      1, DECODE_ID, 0, std::static_pointer_cast<void>(channelTask),

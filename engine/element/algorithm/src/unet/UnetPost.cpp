...
void UnetPost::postProcess(UnetSophgoContext& context,
    common::ObjectMetadatas& objectMetadatas)
    {
        ...
        for(int batch_idx = 0; batch_idx < image_n; ++batch_idx)
        {
            ...
            auto ret = bm_image_create(pSophgoContext->m_bmContext->handle(), height, width, FORMAT_GRAY, DATA_TYPE_EXT_1N_BYTE, result_resized.get());
            ...
            ret = bmcv_image_resize(pSophgoContext->m_bmContext->handle(), 1, &bmcv_resize_attr, &result, result_resized.get());
            assert(BM_SUCCESS == ret);

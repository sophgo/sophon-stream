/**
@file usecaseFaceDetectTrackerQualityFilter.cpp
@brief 主程序
@details
    FaceDetectTrackerQualityFilter集成测试
...
/**
@brief FaceDetectTrackerQualityFilter集成测试函数入口
...
@UnitCase_ID
FaceDetectTrackerQF_IT_0010

@UnitCase_Name
usecaseFaceDetectTrackerQualityFilter

@UnitCase_Description
依次经过解码、retinaFace人脸检测、跟踪、质量过滤、编码、输出worker，检测结果存储在输入objectMetadata的mSubObjectMetadatas字段下的mSpDataInformation中。
...
播放视频，在每一帧都会检测人脸进行跟踪并进行质量过滤，将对应的box绘制在相应位置，播放结束程序可以正常退出
...
//    graphConfigure["modules"].push_back(makeModuleConfig(1, {QUALITY_WORK_ID}, {}, {}));
    ...
    graphConfigure["connections"].push_back(makeConnectConfig(QUALITY_WORK_ID,0,ENCODE_ID,0));

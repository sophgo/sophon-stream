...
float YoloXPost::sigmoid(float x) { return 1.0 / (1 + expf(-x)); }
...
void YoloXPost::postProcess(YoloXSophgoContext& context,
  ...
  for (int batch_idx = 0; batch_idx < pSophgoContext->max_batch; ++batch_idx) {
    ...
    for (size_t i = 0; i < outlen_dim; ++i) {
      ...
      if (box_objectness < pSophgoContext->m_thresh[0]) continue;
      // 进入解码阶段
      float center_x = 0;
      float center_y = 0;
      float w_temp = 0;
      ...
      for (int class_idx = 0; class_idx < pSophgoContext->m_class_num;
           ++class_idx) {
        float box_cls_score = 0;
        if (outputs_3)
          box_cls_score =
              classTensor[classOffset + i * pSophgoContext->m_class_num +
                          class_idx];
        else

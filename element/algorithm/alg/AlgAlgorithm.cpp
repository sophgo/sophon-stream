 ...
 * @return 错误码
 */
common::ErrorCode AlgAlgorithm::initInternal(const std::string& json) {
  ...
  do {
    ...
    mContext = std::make_shared<alg ::AlgSophgoContext>();
    mPreProcess = std::make_shared<alg ::AlgPre>();
    mInference = std::make_shared<alg ::AlgInference>();
    mPostProcess = std::make_shared<alg ::AlgPost>();
    ...
    // mContext->algorithmName = AlgAlgorithm::Name;
    mContext->algorithmName = "Alg";
 ...
 */
void AlgAlgorithm::process(common::ObjectMetadatas& objectMetadatas) {
  common::ErrorCode errorCode = common::ErrorCode::SUCCESS;
  if (use_pre) {
 ...
 * 资源释放函数
 */
void AlgAlgorithm::uninitInternal() {
...
common::ErrorCode AlgAlgorithm::doWork() {
  common::ErrorCode errorCode = common::ErrorCode::SUCCESS;
...
common::ErrorCode AlgAlgorithm::sendProcessedData() {
...
REGISTER_WORKER("Alg", AlgAlgorithm)

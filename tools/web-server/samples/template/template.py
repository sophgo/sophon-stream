...
def template_build_config(algorithm_name,stream_path,data,port,i):
    ...
    json_data["channels"][0]["loop_num"]=-1
    ...
    # 使用 json.load 将文件内容转换为字典
        json_data = json.load(file)
    if("threshold" in data["Algorithm"][i].keys()):
    ...
    if(data["Algorithm"][i]["DetectInfos"]!=None):
        for detectinfoid in range(len(data["Algorithm"][i]["DetectInfos"])):
            ...
            if(head['X']!=0 and head["Y"]!=0 and end['X']!=0 and end["Y"]!=0):
...
def template_trans_json(json_data,task_id,Type,up_list):
    ...
    if("mSubObjectMetadatas" in json_data.keys() and len(json_data["mDetectedObjectMetadatas"])== len(json_data["mSubObjectMetadatas"])):
        for indx in range(len(json_data["mDetectedObjectMetadatas"])):
            tmp=json_data["mDetectedObjectMetadatas"][indx]
    ...
    if len(json_data["mDetectedObjectMetadatas"]) > 0 and len(results["AnalyzeEvents"])==0:
        # 解码base64图片
        image_data = base64.b64decode(src_base64)
        # 使用Pillow打开图片
        image = Image.open(BytesIO(image_data))

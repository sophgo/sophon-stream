...
void YoloXPre::initTensors(YoloXSophgoContext& context,
...
common::ErrorCode YoloXPre::preProcess(
  ...
  // 3. attach to tensor
  //  if(image_n != pSophgoContext->max_batch) image_n =
  //  pSophgoContext->m_bmNetwork->get_nearest_batch(image_n); bm_device_mem_t
  //  input_dev_mem; bm_image_get_contiguous_device_mem(image_n,
  //  pSophgoContext->m_converto_imgs.data(), &input_dev_mem);

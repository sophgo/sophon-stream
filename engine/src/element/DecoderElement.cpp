...
void DecoderElement::onStart() {
    IVS_INFO("DecoderElement start...");
}
...
common::ErrorCode DecoderElement::doWork() {
    ...
    std::shared_ptr<ChannelTask> channelOperate = std::static_pointer_cast<ChannelTask>(data);
    if(channelOperate->request.operation==ChannelOperateRequest::ChannelOperate::START) {
        errorCode = startTask(channelOperate);
    } else if(channelOperate->request.operation==ChannelOperateRequest::ChannelOperate::STOP) {
        errorCode = stopTask(channelOperate);
    } else if(channelOperate->request.operation==ChannelOperateRequest::ChannelOperate::PAUSE) {
        errorCode = pauseTask(channelOperate);
    } else if(channelOperate->request.operation==ChannelOperateRequest::ChannelOperate::RESUME) {
...
common::ErrorCode DecoderElement::startTask(std::shared_ptr<ChannelTask>& channelTask) {
    ...
    int reopentimes = -1;
    int sourceType = 0;
    common::ErrorCode ret = parseJson(channelTask->request.json, url, sourceType, reopentimes);
    ...
    channelInfo->mThreadWrapper->init([this,channelInfo,channelTask]()->common::ErrorCode {
        ...
        IVS_INFO("channel info decoder address: {0:p}", static_cast<void*>(channelInfo->mSpDecoder.get()));

        common::ErrorCode ret = channelInfo->mSpDecoder->init(getSide(),
                                       getDeviceId(),
        ...
        if (common::ErrorCode::SUCCESS != ret) {
            ...
            if(ret==common::ErrorCode::SUCCESS){
                ...
                std::string error = "time out! channel id is "+std::to_string(channelTask->request.channelId);
                channelTask->response.errorInfo = error;
                IVS_ERROR("{0}",error);
                channelInfo->mThreadWrapper->stop(false);
            }
        ...
        return process(false, sourceType, reopentimes, capacity, channelTask, channelInfo);
    }, [this,channelInfo,channelTask,sourceType, reopentimes, capacity]()->common::ErrorCode {
        return process(true,sourceType, reopentimes, capacity, channelTask, channelInfo);
    });
    channelInfo->mThreadWrapper->start();
    std::unique_lock<std::mutex> uq(*channelInfo->mMtx);
    std::cv_status currentNoTimeout = channelInfo->mCv->wait_for(uq, std::chrono::seconds(120));
    if(currentNoTimeout==std::cv_status::timeout) {
        channelTask->response.errorCode = common::ErrorCode::TIMEOUT;
        return common::ErrorCode::TIMEOUT;
    }
    mThreadsPool.insert(std::make_pair(channelTask->request.channelId,channelInfo));
...
common::ErrorCode DecoderElement::stopTask(std::shared_ptr<ChannelTask>& channelTask) {
    std::lock_guard<std::mutex> lk(mThreadsPoolMtx);
    auto itTask =mThreadsPool.find(channelTask->request.channelId);
    if(itTask==mThreadsPool.end()) {
        channelTask->response.errorCode = common::ErrorCode::DECODE_CHANNEL_NOT_FOUND;
        std::string error = "this channel is not found! channel id is "+std::to_string(channelTask->request.channelId);
        IVS_ERROR("{0}", error);
...
common::ErrorCode DecoderElement::pauseTask(std::shared_ptr<ChannelTask>& channelTask) {
    std::lock_guard<std::mutex> lk(mThreadsPoolMtx);
    auto itTask =mThreadsPool.find(channelTask->request.channelId);
    if(itTask==mThreadsPool.end()) {
        channelTask->response.errorCode = common::ErrorCode::DECODE_CHANNEL_NOT_FOUND;
...
common::ErrorCode DecoderElement::resumeTask(std::shared_ptr<ChannelTask>& channelTask) {
    std::lock_guard<std::mutex> lk(mThreadsPoolMtx);
    auto itTask =mThreadsPool.find(channelTask->request.channelId);
    if(itTask==mThreadsPool.end()) {
        channelTask->response.errorCode = common::ErrorCode::DECODE_CHANNEL_NOT_FOUND;
...
common::ErrorCode DecoderElement::parseJson(const std::string& json, std::string& url, int& sourceType, int& reopentimes) {
    common::ErrorCode errorCode = common::ErrorCode::SUCCESS;
    do {
        auto configure = nlohmann::json::parse(json, nullptr, false);
        if (!configure.is_object()) {
        ...
        if (configure.end() == urlIt
                || !urlIt->is_string()) {
            IVS_ERROR("Can not find {0} with string type in element json configure, json: {1}",
                      JSON_URL,
                      json);
            errorCode = common::ErrorCode::PARSE_CONFIGURE_FAIL;
            break;
        ...
        if (configure.end() == reopenIter
                || !reopenIter->is_number_integer()) {
...
common::ErrorCode DecoderElement::process(const bool lastFrame,const int sourceType,const int reopentimes,const int capacity,
    ...
    if(ret==common::ErrorCode::SUCCESS||ret==common::ErrorCode::STREAM_END) {
        ...
        IVS_DEBUG("decode element channel id:{0}--endofstream:{1}", objectMetadata->mFrame->mChannelId,objectMetadata->mFrame->mEndOfStream);

        if(sourceType==2){
            channelInfo->mFrameCount++;
            if(channelInfo->mFrameCount%(mSkipCount+1)!=0&&!objectMetadata->mFrame->mEndOfStream){
                return common::ErrorCode::SUCCESS;
            }
        }
        ...
        //IVS_WARN("decode process output datapipe size is :{0}", outputDatapipeSize);
        if(sourceType==2&&outputDatapipeSize>=capacity){
            IVS_WARN("output datapipe0 size is too high, size is:{0}, capacity is:{1}",outputDatapipeSize, capacity);
            return common::ErrorCode::SUCCESS;
        }
        ...
        if(common::ErrorCode::SUCCESS!=errorCode) {
            ...
            IVS_WARN("Send data fail, element id: {0}, output port: {1}, data: {2:p}", getId(), 0,
        ...
        IVS_ERROR("getFrame failed,error code is : {0}----channel id:{1}",(int)ret,channelTask->request.channelId);
        channelTask->response.errorCode = ret;
...
common::ErrorCode DecoderElement::reopen(const int reopentimes,const std::shared_ptr<ChannelTask>& channelTask, const std::shared_ptr<ChannelInfo>& channelInfo){
    int times = 0;

    common::ErrorCode ret = common::ErrorCode::ERR_FFMPEG_INPUT_CTX_OPEN;
    
    while(channelInfo->mThreadWrapper->mThreadStatus!=ThreadWrapper::ThreadStatus::STOP&&ret!=common::ErrorCode::SUCCESS){
        if(times>=reopentimes&&reopentimes>=0){
            break;
        }

        channelInfo->mSpDecoder->uninit();
        ret = channelInfo->mSpDecoder->init(getSide(),
                                           getDeviceId(),
                                           channelTask->request.json);
        if(ret!=common::ErrorCode::SUCCESS){
            IVS_ERROR("reopen failed! errorcode:{0}, reopentimes:{1}", (int)ret, reopentimes);
            channelTask->response.errorCode = ret;
        }
        if(reopentimes>=0)
            times++;

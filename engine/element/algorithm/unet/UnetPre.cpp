...
void UnetPre::initTensors(UnetSophgoContext& context,
...
common::ErrorCode UnetPre::preProcess(
    UnetSophgoContext& context, common::ObjectMetadatas& objectMetadatas) {
  ...
  for (int i = 0; i < image_n; ++i) {
    ...
    // BGR_PACKED转PLANAR
    bm_image image;
    bm_image_create(pSophgoContext->m_bmContext->handle(), height, width,
                    FORMAT_BGR_PLANAR, DATA_TYPE_EXT_1N_BYTE, &image);
    bmcv_image_storage_convert(pSophgoContext->m_bmContext->handle(), 1,
                               objectMetadatas[i]->mFrame->mSpData.get(),
                               &image);
    ...
    if (need_copy) {
      ...
      bm_image_create(pSophgoContext->m_bmContext->handle(), image.height,
                      image.width, FORMAT_BGR_PLANAR, DATA_TYPE_EXT_1N_BYTE,
    ...
    float ratio =
        get_aspect_scaled_ratio(width, height, pSophgoContext->m_net_w,
    ...
    if (isAlignWidth) {
      ...
      int tx1 =
          (int)((pSophgoContext->m_net_w - padding_attr.dst_crop_w) / 2);  // 0
      padding_attr.dst_crop_sty = 0;
...
#else
    auto ret =
        bmcv_image_vpp_convert(pSophgoContext->m_bmContext->handle(), 1, image,
...
float UnetPre::get_aspect_scaled_ratio(int src_w, int src_h, int dst_w,

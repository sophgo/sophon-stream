...
/**
 * A base class define a mode receive data from input, process the data, and send result to output.
 */
class Element {
public:
     ...
     */
    static void connect(Element& srcElement, 
                        int srcElementPort, 
                        Element& dstElement, 
                        int dstElementPort);
     ...
     */
    void setDataHandler(int outputPort, 
                        DataHandler dataHandler) {
        IVS_INFO("Set data handler, element id: {0:d}, output port: {1:d}",
                 mId, 
                 outputPort);
     ...
     */
    bool getMillisecondsTimeout() const {
    ...
    static constexpr const char* JSON_MILLISECONDS_TIMEOUT_FIELD = "milliseconds_timeout";
...
protected:
    ...
    void popData(int inputPort) {
        IVS_DEBUG("pop data, element id: {0:d}, input port: {1:d}",
                  mId, 
                  inputPort);
    ...
    /** 
     * Send data to output port of this Element, call by doWork() in derived class.
     * @param[in] outputPort : Output port of Element which will send data to.
     * @param[in] data : The data that will be send.
     * @param[in] timeout : The duration that will be wait for.
     * @return If can not find DataHandler or DataPipe on this output port or timeout, it will return error, 
     * otherwise return common::ErrorCode::SUCESS.
     */
    template<class Rep, class Period>
    common::ErrorCode 
    sendData(int outputPort, 
             std::shared_ptr<void> data, 
             const std::chrono::duration<Rep, Period>& timeout) {
        ...
        IVS_ERROR("Can not find data handler or data pipe on output port, output port: {0:d}--element id:{1}", outputPort, mId);
...
private:
    ...
    /**
     * Pending intput notify count, onInputNotify() increase the value and success doWork() decrease the value.
     */
    std::atomic<int> mNotifyCount;

    /**
     * Input DataPipe map of Element, key is input port, value is std::shared_ptr of framework::DataPipe.
     */
    std::map<int/* inputPort */, 
             std::shared_ptr<framework::DataPipe> > mInputDataPipeMap;

    /**
     * Output DataHandler map of Element, key is output port, value is Element::DataHandler.
     */
    std::map<int/* outputPort */, 
             DataHandler> mOutputHandlerMap;
    std::mutex mOutputHandlerMapMtx;

    /**
     * Output DataPipe map of Element, key is output port, value is std::shared_ptr of framework::DataPipe.
     */
    std::map<int/* outputPort */, 
             std::weak_ptr<framework::DataPipe> > mOutputDataPipeMap;
};
